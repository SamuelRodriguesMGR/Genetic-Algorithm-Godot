...
@onready var label_hp : Label = get_node(^"LabelEnergy")
@onready var pushable_component : Node = get_node(^"PushableComponent") 
@onready var brain_component : BrainComponent = $BrainComponent

var energy_points : int = 20: set = set_health

func _ready() -> void:
	label_hp.text = str(energy_points)
...
func update() -> void:
	var action : int = brain_component.predict(sensor_inputs())
	var move_dir : Vector2i = ACTIONS[action]
